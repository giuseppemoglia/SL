...
type variables
char			ic_ExtraChar[30]  // of trim kullan$$HEX1$$3101$$ENDHEX$$ld$$HEX3$$31011f013101$$ENDHEX$$nda at$$HEX1$$3101$$ENDHEX$$lmas$$HEX2$$31012000$$ENDHEX$$istene extra karakterler
end variables
...
public function boolean of_isextrachar (character ac_char)
...
If ab_remove_spaces and ab_remove_nonprint Then
	...
	do while Len (as_source) > 0 and not lb_char
		lc_char = as_source
		if of_IsPrintable(lc_char) and Not of_IsSpace(lc_char) and not of_IsExtraChar(lc_char) then
			lb_char = true
		else
...
public function boolean of_isextrachar (character ac_char);integer	li_ExtChar,li_e	

li_ExtChar = upperbound(ic_ExtraChar[])

for li_e=1 to li_ExtChar
	if ic_ExtraChar[li_e] = ac_char then
		return true		
	end if
next
